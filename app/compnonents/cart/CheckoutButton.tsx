...
export default function CheckoutButton() {
  ...
  return (
    <article className="mt-3 flex flex-col">
      ...
      <button
        onClick={handleClick}
        className="bg-rose-50 hover:bg-rose-500 hover:text-white transition-colors duration-500 text-rose-500 py-3 px-5 rounded-md w-100 disabled:bg-slate-300 disabled:cursor-not-allowed disabled:text-white"
        disabled={(totalPrice && totalPrice < 30) ||
            (cartCount && cartCount > 20) ||
            status == "no-items"
          ? true
          : false}
